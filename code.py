...
def menu_scene():
    ...
    text3 = stage.Text(width=29, height=12, font=None,
                       palette=constants.RED_PALETTE, buffer=None)
    text3.text(
        "Press A to shoot, all buttons on the left side are used to move, press select button to speed up.")
            ...
            if aliens[alien_number].x > 0:
                ...
                if aliens[alien_number].y > constants.SCREEN_Y:
                    aliens[alien_number].move(
                        constants.OFF_SCREEN_X, constants.OFF_SCREEN_Y)
                    show_alien()
        #RST 11, checks collision between sprites
        for laser_number in range(len(lasers)):
                    if lasers[laser_number].x > 0:
                        for alien_number in range(len(aliens)):
                            if aliens[alien_number].x > 0:
                                if stage.collide(lasers[laser_number].x + 6, lasers[laser_number].y + 2, lasers[laser_number].x + 11, lasers[laser_number].y + 12, aliens[alien_number].x + 1, aliens[alien_number].y, aliens[alien_number].x + 15, aliens[alien_number].y + 15):
                                    #RST 11, basically moves the sprite off screen and activates function 
